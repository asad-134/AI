...
def render_main_content():
    ...
    st.markdown('<div class="main-header">🤖 Multi-Agent Analytics Dashboard</div>', 
                unsafe_allow_html=True)
    st.markdown('<div class="sub-header">Powered by Ollama Mistral 7B | Context-Task-Formatting Framework</div>', 
                unsafe_allow_html=True)
...
def render_visualization_creator():
    ...
    with col2:
        use_ollama = st.checkbox("Use AI", value=True, 
                                help="Use Ollama for enhanced generation")
    
    if st.button("🎨 Generate Visualization", type="primary"):
        if user_prompt:
            generate_and_display_chart(user_prompt, use_ollama)
        else:
    ...
    for idx, (label, prompt) in enumerate(quick_prompts):
        with quick_viz_cols[idx]:
            if st.button(label, key=f"quick_{idx}"):
                generate_and_display_chart(prompt, use_ollama)


def generate_and_display_chart(prompt: str, use_ollama: bool = True):
    """Generate and add chart to history"""
    with st.spinner("🎨 Creating visualization..."):
        try:
            # Generate code
            code, method = st.session_state.coordinator.generate_visualization_code(
                prompt, st.session_state.df_clean, use_ollama
            )
